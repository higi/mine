PhoenixMiner 3.5d Documentation
===============================
...
Windows x64 and Linux x64 and has a developer fee of 0.65% (the lowest in the industry).
This means that every 90 minutes the miner will mine for us, its developers, for 35 seconds.
...
Mining options:
  ...
  -gt <n> Set the GPU tuning parameter (6 to 400). The default is 15. You can change the
	  ...
	  the miner will use auto-tuning to determine the best GT value
...
P001: I'm using AMD RX470/480/570/580 or similar card and my hashrate dropped significantly in the past
      ...
      solved by using the special blockchain driver from AMD (or install the latest drivers). For the
      older cards there is no workaround but you still can mine EXP, Musicoin, UBQ or PIRL with the
      same speed that you mined ETH before the drop.
...
P007: PhoenixMiner shows an error after allocating DAG buffer and shuts down (Windows only)
